...
│   ├── ecr.tf               # Configurações para criação do Amazon ECR
│   ├── eks.tf               # Configurações para criação do Amazon EKS
│   ├── security-groups.tf   # Configurações segurança através dos security groups
│   ├── variables.tf         # Definição de variáveis
│   ├── outputs.tf           # Outputs definidos
│   └── provider.tf          # Configuração do provedor AWS
│   ├── vpc.tf               # Configurações da Amazon Virtual Private Cloud (Amazon VPC)
...
A Amazon Virtual Private Cloud (Amazon VPC) permite acesso seguro à infraestrutura AWS permite iniciar os recursos AWS em uma rede virtual logicamente isolada.
